  ...
  return <ul> {
    friends.map((friend) => {
      return <Friend friend={friend} key={friend.id} 
          selectedFriend = {selectedFriend} onSelection={onSelection}/>
    })} </ul>
}
...
function Friend({friend, onSelection, selectedFriend}) {
  ...
  return <li className={isSelected ? "selected" : ""}> 
    ...
    {friend.balance === 0 && <p className="blue"> You and {friend.name} are even.. </p>}
...
function FormSplitBill({selectedFriend, onSplitBill}) {
  ...
  <input type="text" value = {paidByUser} 
    onChange={(e)=> {
    return setPaidByUser(Number(e.target.value) > bill ? paidByUser : Number(e.target.value));
  }}/>
